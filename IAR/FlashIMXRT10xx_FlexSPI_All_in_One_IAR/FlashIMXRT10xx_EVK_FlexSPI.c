...
#endif /* USE_ARGC_ARGV */
{
  ...
  for(int i = 0; i < argc; /*i++*/)
  {
    ...
    else
    {
      ...
      else if(strcmp("--BT_FUSE", argv[i]) == 0)
      {
        ...
        OCOTP_ReadFuseShadowRegisterExt(OCOTP, 0x06, &fuseData, 1);
		/* Set BT_FUSE_SEL = 1, boot options are controlled by 
           the eFuses instead of GPIO CFG pins */
        if((fuseData & (1<<4)) == 0){
          fuseData |= 1<<4;//Enable BT_FUSE_SEL
          OCOTP_WriteFuseShadowRegister(OCOTP, 0x06, fuseData);
        }
      }
